...
学习目标：
- 从已有 Python 数据创建数组。
- 使用 zeros、ones、full、eye 创建特定形状的数据。
- 分清 arange 的“步长”和 linspace 的“数量”。
- 理解 dtype 会影响精度、内存以及可以保存的值。

创建数组时不要只看数据本身，还要同时确认 shape 和 dtype。许多后续问题不是
计算公式错了，而是数组形状或数据类型在创建时就不符合预期。
"""
