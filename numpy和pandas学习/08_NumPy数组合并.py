...
合并前先看 shape。concatenate 会沿已有轴延长数组，除目标轴外的其他维度必须
一致；stack 会创建一个新的轴，因此要求所有输入数组的完整 shape 一致。

常用选择：
- vstack：上下堆叠，二维时相当于增加行。
- hstack：左右拼接，二维时相当于增加列。
- concatenate：明确指定 axis，控制最直接。
- stack：保留每个输入数组的整体身份，新增一层维度。

一维数组的 shape 是 (n,)，没有真正的行列。需要行向量或列向量时，用 newaxis
或 reshape 显式增加维度。
"""
