...
设置值与选择数据使用同一套索引规则。推荐一次性写出行和列，例如
frame.loc[条件, "列名"] = 新值，避免 frame["列名"][条件] 这种链式赋值。

本集需要理解：
- loc/iloc 修改已有位置，at/iat 适合单个值。
- 给整列赋标量时，标量会广播到所有行。
- 给整列赋 Series 时，Pandas 按索引标签对齐，而不是只按位置复制。
- assign 和 drop 默认返回新表，便于保留原始数据并组织处理流程。
"""
