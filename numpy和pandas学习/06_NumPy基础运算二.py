...
学习重点是 axis。可以把 axis 理解成“本次运算要折叠掉的轴”：
- 二维数组 axis=0 折叠行，同一列的数据参与运算，结果按列产生。
- 二维数组 axis=1 折叠列，同一行的数据参与运算，结果按行产生。

argmin/argmax 返回位置，min/max 返回数值；cumsum 返回累计值，diff 返回相邻
元素的变化量。keepdims=True 可以保留被折叠的维度，方便结果继续参与广播。
"""
